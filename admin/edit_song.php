...
include 'header.php';
include 'banner.php';
$edit_song=mysqli_query($con, "select * from song where id= '$_GET[id]'");
...
<div class="container mt-3">
    ...
    while($row_genre=mysqli_fetch_array($run_genre)){
	  ...
	  if($fetch_edit['genre_id']==$genre_id){
		  echo "<option value='$fetch_edit[genre_id]' selected>$genre_name</option>";
	  }
	  else{
    ...
    $run_artist = mysqli_query($con, $get_artist);
		...
		if($fetch_edit['artist_id']== $artist_id){
			echo "<option value ='$fetch_edit[artist_id]' selected>$artist_name</option>";
		}
		else{
    ...
    <tr>
        <td valign="top"><b>Date Release:</b></td>
        <td><input type="date" name="date_release" value='<?php echo $fetch_edit['date_release'];?>'></td>
    </tr>
    <tr>
...
$update_query = "update  song set name='$name', genre_id='$genre', artist_id='$artist', image='$image', sound='$file',date_release='$date',price='$price' where id = '$_GET[id]'";
